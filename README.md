...
- `syntheticAuthRefs` + `prepareSyntheticAuth` probe of `cursor-agent status --format json`, so Control UI model picks stay in chat instead of bouncing to model-setup/settings when Cursor is already logged in
